...
//    ]);
    ...
    Route::group(['prefix' => LaravelLocalization::setLocale(), 'middleware' => ['localizationRedirect']], function () {
        Route::group(['prefix' => 'auth', 'namespace' => 'Site'], function () {
            ...
            Route::get('/sms/verification/{user_id}', ['uses' => 'AuthController@verificationSMS'])->name(
                'sms.verification'
            );
            ...
            Route::post('consent/data', ['uses' => 'AuthController@agreeConsentToDataCollection'])->name(
                'consent.data'
            );
        ...
        Route::group(['namespace' => 'Site\Registrations', 'prefix' => 'registrations'], function () {
            ...
            Route::get('/restore/verify-{verify}', 'RegistrationsController@update')->name(
                'registrations.restore.update'
            )->where('verify', '(.*)');
            Route::post('/restore/verify-{verify}', 'RegistrationsController@save_new_password')->name(
                'registrations.restore.save_pass'
            )->where('verify', '(.*)');
        });
        ...
        Route::group(['namespace' => 'Site'], function () {
            ...
            Route::group(['namespace' => 'Sections'], function () {
                ...
                Route::get('news/{alias}/rubrics/{rubric}', 'NewsController@rubricsShow')->name(
                    'site.news.rubrics.show'
                )->where('rubric', '[0-9]+');

                Route::get('gallery/{alias}', 'GalleryController@index')->name('site.gallery.index');
                Route::get('gallery/{alias}/{id}', 'GalleryController@show')->name('site.gallery.show')->where(
                    'id',
                    '[0-9]+'
                );
                ...
                Route::get('services/{alias}/{id}', 'ServicesController@show')->name('site.services.show')->where(
                    'id',
                    '[0-9]+'
                );
                ...
                Route::get('objects/{alias}/{object}', 'ObjectsController@show')->name('site.objects.show')->where(
                    'object',
                    '[0-9]+'
                );
            ...
            Route::group(['namespace' => 'Cabinet'], function () {
                ...
                Route::post('cabinet', 'CabinetController@saveRelevantProfileData')->name(
                    'cabinet.check-profile-relevance'
                );

                Route::get('cabinet/{id}/check_grafic', 'CabinetController@CheckGrafic')->name('cabinet.checkgrafic');
                Route::get('cabinet/{id}/check_grafic/pdf', 'CabinetController@GraficPDF')->name(
                    'cabinet.checkgrafic.pdf'
                );
...
//            Route::get('cabinet/{id}/feedback', 'CabinetController@feedback')->name('cabinet.feedback');
                Route::get('cabinet/{id}/feedback_template', 'CabinetController@feedback_template')->name(
                    'cabinet.feedback'
                );
                Route::get('cabinet/{id}/feedback_template/view', 'CabinetController@view_template');
                Route::post('cabinet/{id}/feedback_send', 'CabinetController@feedback_send')->name(
                    'cabinet.feedback.send'
                );
            });
        });
    });
