$(document).ready(function() {
	...
	$("#sortable").sortable({
			...
			stop: function(event, ui) {
					...
					$.get('/admin/ajax/'+ $("#section_id").val() +'/objects-media-sortable?' + sort, {
	...
	$('#upload--photos').uploadifive({
			...
			'uploadScript'	: '/admin/ajax/object-images',
			'fileType'		: 'image/*',
			'formData'		: {
					'_token'      : $('meta[name="_token"]').attr('content'),
	...
	$('#upload--files').uploadifive({
				...
				'width'			: '100%',
				'checkScript'	: '/admin/ajax/check',
				'uploadScript'	: '/admin/ajax/object-files',
				'folder'		: '/uploads/tmps/',
	...
	$("body").on('click', '.save--video_link', function(e) {
		...
		$.ajax({
				url: '/admin/ajax/saveObjectVideoLink',
				type: 'POST',
				async: false,
				dataType: 'json',
				data : { _token: $('meta[name="_token"]').attr('content'), title: title, lang: lang, link: link, id: id, object_id: object_id},
				success: function(data) {
					if (data.errors) {
						messageError(data.errors);
					} else {
